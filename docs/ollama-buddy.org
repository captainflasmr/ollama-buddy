...
- [[#RAG][25 RAG (Retrieval-Augmented Generation)]]
  ...
  - [[#Inline-_0040rag_0028_0029-Syntax][25.9 Inline @rag() Syntax]]
  - [[#Example-Workflow][25.10 Example Workflow]]
...
- =ollama-buddy-rag-cancel (==c==)= :: Cancel an in-progress indexing or
  update operation. The cancellation takes effect before the next
  embedding batch is sent, so the current in-flight batch completes but
  no further batches are processed. The previous index (if any) is left
  unchanged.
...
*** 25.9 Inline @rag() Syntax [[#Inline-_0040rag_0028_0029-Syntax][¶]]
:PROPERTIES:
...
<<Example-Workflow>>
*** 25.10 Example Workflow [[#Example-Workflow][¶]]
:PROPERTIES:
