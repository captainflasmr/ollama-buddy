...
- [[#Remote-Providers][17 Remote Providers]]
  ...
  - [[#Provider-Configuration][17.5 Provider Configuration]]
    ...
    - [[#Generic-OpenAI_002dCompatible-Server-Configuration][17.5.7
...
- [[#Ollama-Cloud-Models][18 Ollama Cloud Models]]
  ...
  - [[#Cloud-Usage-Stats][18.7 Cloud Usage Stats]]
  - [[#Configuration-3][18.8 Configuration]]
...
(default 120 seconds, configurable via =ollama-buddy-multishot-timeout=).
If a model exceeds the timeout, its request is cancelled, a =TIMED OUT=
marker is inserted, and the sequence continues with the next model.
Set =ollama-buddy-multishot-timeout= to =nil= to disable.

**** Benchmark All Models

Press =C-c u= (or use the transient menu: =u Benchmark All=) to
automatically send a prompt to every available model via multishot.  This
builds persistent token usage statistics for all accessible models in one
command.  Embedding models are filtered out.
...
- =l= - Generic OpenAI-compatible server provider loaded
...
**** 17.5.7 Generic OpenAI-Compatible Server Configuration [[#Generic-OpenAI_002dCompatible-Server-Configuration][¶]]
:PROPERTIES:
...
<<Cloud-Usage-Stats>>
*** 18.7 Cloud Usage Stats [[#Cloud-Usage-Stats][¶]]
:PROPERTIES:
:CUSTOM_ID: cloud-usage-stats
:CLASS: section
:END:
The Model Management page (=C-c M=) can display your Ollama cloud usage
(session and weekly percentages) as visual progress bars in the Cloud
Models section.
...
<<Configuration-3>>
*** 18.8 Configuration [[#Configuration-3][¶]]
:PROPERTIES:
:CUSTOM_ID: configuration-1
:CLASS: section
:END:
