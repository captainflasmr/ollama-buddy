...
,#+begin_example
┌───────────────────────────────────┐
│  O L L A M A B U D D Y  [v5.0.1]  │
└───────────────────────────────────┘
,#+end_example

o: Ollama (5)             u: Cloud (2)
...
Open the Token Usage stats from the menu (=C-c #=)
...
- Use =C-c A= to open Auth menu and sign in to Copilot
...
- Attach multiple files with =C-c C-a= (or =C-c a= for the attachment menu)
...
#+begin_comment
The following org-babel blocks generate live tables but are kept as a comment
since the results go stale.  Evaluate them in Emacs to get up-to-date listings.
...
| ollama-buddy-transient-menu       | Wrapper function for safely loading the Ollama Buddy transient menu.           |
...
*** DONE Cloud Models Support

Ollama v0.12+ cloud-hosted models are fully supported:
...
- =ollama-buddy-cloud-signin= / =ollama-buddy-cloud-signout= / =ollama-buddy-cloud-status=
- Auto-pull manifest for new cloud models
- Model management buffer shows cloud models with auth status

*References:* [[https://ollama.com/blog/cloud-models][Ollama Cloud Models Blog]]

*** DONE Tool Calling / Function Calling

Full tool calling support via =ollama-buddy-tools.el= (v2.0.0):
- Built-in tools: =read_file=, =list_directory=, =get_buffer_content=, =list_buffers=, =calculate=, =search_buffer=, =write_file=, =execute_shell=
- =ollama-buddy-tools-register= API for custom tool registration with JSON schema parameters
- Safe mode (default: on) restricts to read-only tools
- =ollama-buddy-tools-toggle-safe-mode= to enable write/execute tools
- ⚒ indicator in header line for tool-capable models
- =ollama-buddy-tools-models= customization for tool-capable model list
- Automatic multi-turn tool execution with configurable iteration limit

*References:* [[https://ollama.com/blog/tool-support][Ollama Tool Support Blog]]

*** DONE Web Search API Integration

Full implementation in =ollama-buddy-web-search.el=:
- =ollama-buddy-web-search-api-key= customization
- Inline search syntax: =@search(query)= in prompts
- =C-c / s= search and display, =C-c / a= search and attach
- Status line shows =♁N= indicator for attached web searches
- Content fetching via eww/shr for clean text extraction
- Works with all providers (local and remote)

*References:* [[https://ollama.com/blog/web-search][Ollama Web Search Blog]]

*** DONE Enhanced Token Metrics

Token metrics from API responses are fully parsed and displayed:
- =prompt_eval_count= and =eval_count= tracked per request
- Token rate (tokens/sec) calculated and displayed
- =C-c #= shows comprehensive token statistics with graphs
- Real-time token display during streaming (toggle with =C-c T=)
- Historical token usage tracking per model

*** DONE Think Parameter for Reasoning Models

Reasoning/thinking model support:
- =ollama-buddy-hide-reasoning= toggle (=C-c V=) to show/hide thinking blocks
- =ollama-buddy-reasoning-markers= customization for different model formats
- Header line shows =V= indicator when reasoning is hidden
- Works with DeepSeek-R1, QwQ, and other reasoning models

*** DONE Vision/Image Support

Full multimodal support:
- Base64 image encoding for vision-capable models
- Auto-detection of image files in prompts (file paths, URLs)
- ⊙ indicator for vision-capable models
- =ollama-buddy-vision-models= customization
- Works with llava, gemma3, and other multimodal models
