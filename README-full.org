...
*** DOING Web Search API Integration
...
*Implemented in =ollama-buddy-web-search.el=:*
- =ollama-buddy-web-search-api-key= customization
- =ollama-buddy-web-search= - interactive search with results display
- =ollama-buddy-web-search-attach= - search and attach results to context
- =ollama-buddy-web-search-detach= / =ollama-buddy-web-search-clear= - manage attached searches
- Status line shows =♁N= indicator for attached web searches
- Token estimation for context budget awareness
- Keybindings: =C-c / s/a/d/l/c=
- =ollama-buddy-web-search-content-source= - choose between ='eww= (default, fetches URLs via eww/shr) or ='api= (uses Ollama API content directly)
