...
- =C-c o= opens the dynamic role menu
...
- *Toggle History (J)*: Enable/disable conversation history
- *Clear History (X)*: Clear the current history
- *Edit History (H)*: Edit the conversation history in a buffer
...
C  Context Info            >  History Display
...
Roles
[Roles]
R  Switch Role             D  Open Directory
#+end_src
...
y  Typical P                w  Penalize Newline
r  Repeat Penalty
...
[Profiles]                  [Actions]
d  Default Profile          l  Display All
...
1. Run =M-x ollama-buddy-roles-switch-role= or use the transient menu
...
- =C-c J=: Toggle history tracking on/off
- =C-c X=: Clear the current conversation history
- =C-c H=: Edit conversation history
...
- The =:destination= property controls where the response goes: =chat= (always chat buffer), =in-buffer= (always replace selected region), or =nil= (honour the global in-buffer replace toggle).
...
- =nil= (default) — honours the global in-buffer replace toggle
...
Cloud models are discovered automatically from =ollama-buddy-cloud-models-url= (default: =https://ollama.com/api/tags=) on first access.  The list is session-cached; use =M-x ollama-buddy-cloud-sync-models= or =/sync= to force a refresh.
