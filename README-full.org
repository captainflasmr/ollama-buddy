...
When called, the main transient menu appears at the bottom of your Emacs frame, organized into a compact 4x2 grid for efficient access. Here's what you'll see:

#+begin_src 
|o(Y)o| Ollama Buddy
[Chat]              [Model]                [Actions]              [Tools]
o  Open Chat        W  Manage Models       r  Refactor Code       f  Fabric Patterns
O  Commands         m  Switch Model        d  Describe Code       u  Universal Patterns
RET Send Prompt     c  Cloud Models        g  Git Commit Message  a  Awesome Prompts
k  Kill/Cancel      i  Show Model Info     l  Lookup Definition   w  Web Search
x  Toggle Streaming M  Multishot           p  Proofread Text      t  Ollama Tools
h  Help/Menu        U  Check Status        c  Custom Prompt       r  RAG Integration

[History]           [Sessions]             [Sub-menus]            [General]
H  Toggle History   N  New Session         S  Settings             q  Quit
X  Clear History    L  Load Session        P  System Prompts       ?  Help
J  Edit History     S  Save Session        R  Roles Management     B  Intro Screen
                    Q  List Sessions                              V  Version Info
                    D  Open Directory                             z  About
#+end_src
...
- *Manage Models (W)*: Info, Pull, Copy, Delete for each model, plus Import GGUF Model and Custom Pull from ollama library
...
- *Check Status (U)*: Checks the status of the local Ollama server

*** Actions Section

Quick access to common AI-powered text operations:

- *Refactor Code (r)*: Suggest improvements to selected code
- *Describe Code (d)*: Explain the selected code snippet
- *Git Commit Message (g)*: Generate a commit message from changes
- *Lookup Definition (l)*: Look up a word or concept
- *Proofread Text (p)*: Check for grammar and style
- *Custom Prompt (c)*: Enter a one-off custom prompt

*** Tools Section

Integration with external knowledge and advanced features:

- *Fabric Patterns (f)*: Access curated prompt templates from Fabric
- *Universal Patterns (u)*: Generic patterns applicable to many situations
- *Awesome Prompts (a)*: Access the Awesome ChatGPT Prompts collection
- *Web Search (w)*: Search the web and integrate results
- *Ollama Tools (t)*: Use tool-calling capabilities (if supported)
- *RAG Integration (r)*: Search your local knowledge base
...
- *List Sessions (Q)*: Browse your saved sessions
- *Open Directory (D)*: Open the sessions directory in Dired

*** Sub-menus Section

Organized sub-menus for specialized settings:

- *Settings (S)*: Configure buffer behavior, display toggles, and UI options
- *System Prompts (P)*: Manage your personal library of system prompts
- *Roles Management (R)*: Create and switch between different AI roles

*** General Section

Global commands and information:

- *Quit (q)*: Close the transient menu
- *Help (?)*: Access full documentation
- *Intro Screen (B)*: Show the welcome tutorial
- *Version Info (V)*: Display the current package version
- *About (z)*: Information about Ollama Buddy

** Specialized Sub-menus

*** Settings Sub-menu

Accessed with *S* from the main menu, this organizes configuration options:

#+begin_src 
Ollama Buddy Settings
[Buffer Options]           [Display Toggles]          [Interface]
C  Clear Buffer            T  Token Display           A  Toggle Level
o  Toggle Markdown->Org    B  Debug Mode              z  Toggle Full Welcome
s  Scroll to Bottom        #  Token Stats             r  Reset Display
w  Wrap Prefix             v  Reasoning Visibility
#+end_src

*** System Prompts Sub-menu

Accessed with *P* from the main menu, this handles your prompt library:

#+begin_src 
System Prompts
[Management]               [Active Prompt]            [Actions]
L  List All Prompts        s  Set Current Prompt      q  Back to Main Menu
E  Edit Prompt             r  Reset System Prompt
S  Save Current as New     C-s Show Active Prompt
#+end_src

*** Roles Management Sub-menu

Accessed with *R* from the main menu, this manages AI roles:

#+begin_src 
Roles Management
[Actions]                  [Organization]             [Navigation]
s  Switch Role             d  Open Roles Directory    q  Back to Main Menu
c  Create New Role         r  Refresh Roles
e  Edit Current Role
#+end_src
