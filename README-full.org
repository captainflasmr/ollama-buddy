...
- Cloud models (=cl:= prefix) with Select and Pull Manifest actions, plus auth status
...
** <2026-02-12 Wed> *2.0.0*

Tool Calling Support and Refined Model Indicators

- New =ollama-buddy-tools.el= module enabling LLMs to invoke Emacs functions during conversations
- Built-in tools: =read_file=, =list_directory=, =get_buffer_content=, =list_buffers=, =calculate=, =search_buffer=, =write_file=, =execute_shell=
- Registration API (=ollama-buddy-tools-register=) for adding custom tools with JSON schema parameters
- Safe mode (default: on) restricts to read-only tools; toggle with =ollama-buddy-tools-toggle-safe-mode=
- Toggle tool calling on/off with =C-c W= (=ollama-buddy-tools-toggle=)
- List registered tools with =C-c Q= (=ollama-buddy-tools-info=) in an org-mode formatted buffer
- Both commands available in transient menu under Actions (=W= and =Q=)
- Tools are only sent to models that support them (no error if model doesn't support tools)
- ⚒ indicator in header line shows when tools are *enabled* AND model supports tools
- ⚒ indicator in prompt line shows when model *supports* tool calling
- ☁ indicator shown consistently in header, prompt, completing-read, and model management
- ☁ and ⚒ indicators combined in prompt line for cloud models with tool support
- Model prefixes (=o:=, =cl:=) only used when external providers are loaded
- Welcome screen omits provider summary when only Ollama (local + cloud) is used
- New =ollama-buddy-tools-models= customization for configuring which models support tool calling

** <2026-02-12 Wed> *1.5.0*

Dynamic Transient Role Menu with Grouped Commands

- New =ollama-buddy-role-transient-menu= command displays role commands in a full transient popup instead of the minibuffer menu
- New =:group= property on command definitions controls transient column grouping
- Recommended binding is now =("C-c o" . ollama-buddy-role-transient-menu)=
- The original =ollama-buddy-menu= remains available as a minibuffer fallback
- Commands without =:group= fall into a single "Commands" column (backwards compatible)
- Commands with =:model= show the model name in the transient description
- All 10 built-in presets updated with =:group= properties for organized column layouts

** <2026-02-12 Wed> *1.4.0*

Tone Selector for Response Style

- New tone selector (=C-c ~= or transient menu =~=) to quickly switch response style
- Five built-in tones: Normal, Concise, Learning, Explanatory, Formal
- Tone text prepended to the global system prompt for consistent styling
- Customizable via =ollama-buddy-tone-alist=
- Header line shows =~X= indicator when a non-Normal tone is active (e.g. =~C= for Concise)

** <2026-02-12 Wed> *1.3.5*

Cloud Models in Model Management and Categorized Recommendations

- Model Management (=C-c M=) now shows cloud models in a dedicated =☁ Cloud Models= section with auth status and Pull Manifest actions
- Recommended models reorganized into foldable categories: General Chat, Reasoning, Efficient & Capable, Coding, General Alternatives
- Each category includes a description to help new users choose the right model type
- Local models no longer show =o:= prefix unless external providers are loaded

** <2026-02-11 Tue> *1.3.4*

Header Line and Interface Refinements

- Streamlined header line: removed ORG/MD indicator, provider indicators, and verbose reasoning text
- History indicator off by default; =<= shows when global prompt disabled; =V= for hidden reasoning
- Transient menu: model management on =M=, multishot on =U=, added ORG/MD and global prompt toggles
- Simplified session management: single Directory command replaces List + Delete
- Web search module auto-loaded; smart =C-a= in chat; updated curated models list
- =ollama-buddy-full-welcome-enabled= variable for controlling welcome screen detail

** <2026-02-10 Mon> *1.3.3*

Auto-Pull Cloud Model Manifests

- Cloud models that haven't been used before now get their manifest pulled automatically
- No more confusing "model not found" errors when selecting a new cloud model
- Works with both network-process and curl backends

** <2026-02-06 Thu> *1.3.2*

New Role Presets: Tutor and Documenter

- *Tutor*: Educational assistant with explanation commands (eli5, analogies, step-by-step), assessment (quizzes, practice problems), and Emacs Info integration with completing-read for browsing info nodes
- *Documenter*: Technical writing assistant for READMEs, API docs, docstrings, changelogs, FAQs, and troubleshooting guides

** <2026-02-06 Thu> *1.3.1*

Role Menu Improvements

- Per-role =ollama-buddy-menu-columns= - each role preset can define its own menu column layout
- Fixed menu display truncation when rows exceeded =max-mini-window-height=
- Streamlined role presets by removing less commonly used commands

** <2026-02-05 Wed> *1.3*

Added Web Search Integration with Ollama's Web Search API

- New =ollama-buddy-web-search.el= module for real-time web search capabilities
- Search results are fetched via Ollama's REST API, URLs are processed through Emacs' eww/shr for clean text extraction, then passed to your selected LLM
- Inline search syntax: use =@search(your query)= directly in prompts for automatic search and context attachment
- Manual search commands: =C-c / s= (search & display), =C-c / a= (search & attach)
- New transient menu accessible via =C-c O /= for web search operations
- Web searches are treated as attachments - cleared together with =C-c 0=
- Status line shows ♁N indicator when web searches are attached
- Works with all providers: Ollama, OpenAI, Claude, Gemini, Grok, Copilot, and Codestral

To use web search:
1. Get an API key from https://ollama.com/settings/keys
2. Configure: =(setq ollama-buddy-web-search-api-key "your-key")=
3. Use =@search(query)= in prompts or =C-c / a= to attach search results
...
(use-package ollama-buddy
  ...
  ;; Content source: 'eww (default, recommended) or 'api (faster, experimental)
  (setq ollama-buddy-web-search-content-source 'eww)
  (require 'ollama-buddy-web-search nil t))
#+end_src

Content source options:
- =eww= (default) - Fetches URLs and renders through eww/shr for cleaner content
- =api= - Uses Ollama API content directly (faster but less refined)

** <2026-02-02 Sun> *1.2.1*

Added attachment count indicator to header-line - displays ≡N when files are attached for constant visual reminder.
...
- Supports OpenAI (gpt-4o, gpt-5), Anthropic (claude-sonnet-4, claude-opus-4.5), Google (gemini-2.5-pro), and xAI models
...
- Moved "Recommended Models (Pull)" section into =C-c M= model management buffer
- Removed =C-c A= (Detailed Info) keybinding and =ollama-buddy-show-interface-info= function
- Updated welcome screen to reference =C-c M= for model management
...
- Model management via =C-c M= with recommended models to pull
...
- *Interactive Command Menu*
  ...
  - Minibuffer fallback menu still available via =M-x ollama-buddy-menu=
...
RET Send Prompt    C-s Show System      c  Cloud Models        D  Open Roles Directory
...
#  Token Stats                                    Q  List Sessions       K  Reset Parameters
C-o Toggle Markdown->Org                          Z  Delete Session      F  Toggle Params in Header
V  Toggle reasoning visibility                                           p  Parameter Profiles                                           
#+end_src
...
- *Manage Models (M)*: Info, Pull, Copy, Delete for each model, plus Import GGUF Model and Custom Pull from ollama library
- *Switch Model (m)*: Changes the active LLM
- *Cloud Model (c)*: Switch to a cloud model
- *Show Model Info (i)*: Displays detailed information about the current model
- *Multishot (U)*: Sends the same prompt to multiple models
...
- *Token Stats (#)*: Show detailed token usage statistics with graphs
- *Context Info (C)*: Display context usage information
- *Toggle ORG/MD (C-o)*: Enable/disable conversion to Org format
- *Context Display (%)*: Toggle context percentage display
- *Context Type (&)*: Toggle context display type
- *History Display (>)*: Toggle history indicator in header
- *Token Display (T)*: Show/hide token usage statistics
- *Reasoning (V)*: Hide/show reasoning/thinking sections
- *Global Prompt (<)*: Toggle global system prompt
...
- *Directory (Z)*: Open sessions directory in Dired
...
Use =C-c M= to access the model management interface with all available actions.
