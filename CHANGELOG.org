...
Thinking Stream Visibility.

*** New Features

- *Thinking Stream Visibility* -- Toggle live viewing of thinking tokens as they
  stream in with =C-c V=.  When enabled, thinking content is displayed under the
  =*** Think= heading in real time, then replaced with the converted org content
  and folded on completion.  A =◐= indicator appears in the header line when active.
