...
- Replaced custom overlay approach with native org/outline folding (=outline-hide-subtree= / =outline-show-subtree=), eliminating all custom keymaps, text-property tracking, and arrow-char manipulation from the previous implementation.
...
- *Header line spacing* -- Added a space between the status message and the system prompt indicator in the header line.
