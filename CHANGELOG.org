...
- *=/= Slash Commands* -- Typing =/= in the prompt area triggers =completing-read= with action commands (=model=, =system=, =clear=, =save=, =load=, =tools=, =context=, =help=). Commands execute immediately without sending text to the LLM. Filtered by loaded modules (=system= requires =ollama-buddy-user-prompts=, =tools= requires =ollama-buddy-tools=). Outside the prompt area, =/= inserts literally. Press =C-g= to cancel and insert a literal =/=.
