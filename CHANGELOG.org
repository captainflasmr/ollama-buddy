...
- *=/new=, =/exit=, and =/unload= slash commands* -- =/new= starts a fresh session
  (same as =/clear=), =/exit= closes the chat buffer, =/unload= lets you pick a
  running model to unload from memory (or unload all at once via =[All]=).
