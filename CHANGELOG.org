...
- =ollama-buddy-rag-cancel= is now an alias for =ollama-buddy-rag-pause=.
  Cancelling saves partial progress instead of discarding it.
- Cloud Models section now appears before Local models in the Model Management
  buffer.
