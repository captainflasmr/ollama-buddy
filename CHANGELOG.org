...
- *Gemma 4 Cloud Model* -- Added =gemma4:31b-cloud= to
  =ollama-buddy-cloud-models=, so it appears in the =C-c m= model picker
  with the =☁= indicator.
- *Gemma 4 Recommended Models* -- Added =gemma4:e2b=, =gemma4:e4b= and
  =gemma4:latest= to the "Efficient & Capable" category of
  =ollama-buddy-available-models= (shown via =C-c L=).  The larger
  =gemma4:26b= / =gemma4:31b= variants are intentionally left out since
  the recommended list targets models that fit on a regular consumer
  PC.  =gemma2:27b= was dropped from the same list for consistency.
