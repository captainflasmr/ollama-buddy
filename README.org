...
- *Role Switching*
  ...
  - create, switch, and manage role-specific command menu configurations
...
- *Real-time token counting:* As responses are streamed, the system tracks the number of tokens generated.
- *Token rate calculation:* The average tokens per second are computed dynamically.
- *Background updates:* A timer updates the token count and rate periodically.
- *Detailed token history:* A history of token usage across interactions is maintained.
- *User-configurable display:* Users can enable or disable token statistics in responses.
- *Statistics view:* A new command shows an overview of token usage over time.

*** How It Works

When a response starts, Ollama Buddy initializes tracking variables and starts a timer. As tokens are received, the counter updates, and the current rate is computed. Once the response completes, the total token count and rate are added to a history log. If enabled, token statistics are displayed at the end of responses.

A new command, ~M-x ollama-buddy-display-token-stats~, provides a summary of token usage, including per-model breakdowns and recent history.

*** Enabling and Disabling Token Display

Users can toggle automatic token statistics display by calling:

#+BEGIN_SRC emacs-lisp
M-x ollama-buddy-toggle-token-display
#+END_SRC

This setting persists during the session, allowing users to view or hide token details as needed.

*** Improving Your Workflow

This feature provides better insights into model performance and helps users optimize their interactions with Ollama Buddy. Whether monitoring efficiency or benchmarking responses, real-time token tracking enhances usability.

** <2025-02-28> *0.2.4*

Added model-specific color highlighting (experimental - off by default)
...
1. *Model-Specific Colors (Experimental)*
...
This feature is currently *experimental*, and feedback is welcome to refine its implementation further!
...
| ollama-buddy-enable-model-colors       | Whether to show model colors. EXPERIMNTAL.                    |
...
** TODO Model hash colouring algorithm to ensure the contrast is sufficient and hence visibility maintained with differing themes.
