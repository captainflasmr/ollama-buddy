...
- Manage models          C-c W
- Select model           C-c m (10 ollama / 4 online)
- Select cloud model     C-u C-c m (7 cloud)
...
Press =C-c W= to open the =*Ollama Models Management*= buffer which shows:
...
- Model management via =C-c W= with recommended models to pull
...
- *Interactive Command Menu*
  ...
  - Comprehensive model management with =C-c W=
...
- *Manage Models (W)*: Info, Pull, Copy, Delete for each model, plus Import GGUF Model and Pull any model from ollama library
...
Use =C-c W= to access the model management interface with all available actions.
...
- Full model information is available in the model management buffer (=C-c W=)
