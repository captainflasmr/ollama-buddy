...
| TODO                                                                         | DOING                                      | DONE                                                                  |
|------------------------------------------------------------------------------+--------------------------------------------+-----------------------------------------------------------------------|
| sometimes chat output with filter is getting clobbered                       | Kill online AI process with cancel request | chat ollama names to include prefix                                   |
| Modifying a parameter internal defaults would flag as not modified           | GGUF to use new syncing option             | Still some encoding issues on returns from online AI                  |
| transient-setup: Key sequence 1 0 starts with non-prefix key 1               |                                            | Conversation history to display with Claude/ChatGPT                   |
| No streaming with hiding reasoning corrupts output                           |                                            | I think some hard coded Claude models currently don't work            |
| Claude request and connection can sometimes take time and it blocks!         |                                            | register contents to be converted to org, currently it is in markdown |
| Pushing hundreds or even thousands of lines to the chat buffer takes a while |                                            | =C-a= beginning-of-line to jump to prompt start like comint           |
| ChatGPT request/response too long                                            |                                            | As shell generally C-n to end of list to clear prompt history         |
| Check tags caching                                                           |                                            | Add Texinfo to MELPA recipe                                           |
| Save system prompts as part of session                                       |                                            | System prompt not cleared on new session                              |
| keybindings in ollama chat buffer are reserved                               |                                            | Remove fabric from Commands transient menu                            |
...
** TODO sometimes chat output with filter is getting clobbered
...
| TODO                                                       | DOING                                                         | DONE                                         |
|------------------------------------------------------------+---------------------------------------------------------------+----------------------------------------------|
| check context size of current model when querying          | Add option to hide reasoning in chat buffer                   | Show token parameters as part of token stats |
| Autoload intelligently? to speed up initial reponse times? | Unload all models?, to free up RAM if you are switching alot? | Autogenerate session save name               |
| Core API - Embeddings                                      | Core Params - suffix                                          | Add gemini?                                  |
| Core Params - format                                       |                                                               | Pull ChatGPT and Claude model names          |
| Core Params - template                                     |                                                               | Merge history buffer functionality           |
| Core Params - raw                                          |                                                               | Show incoming ollama messages                |
| Test on Windows                                            |                                                               | Add chatGPT awesome prompt support           |
...
** DOING Unload all models?, to free up RAM if you are switching alot?

** TODO check context size of current model when querying
...
** DONE 001.1 - Toggle reasoning/chain of thought
CLOSED: [2025-04-26 Sat 08:12]

** DONE 001.2 - Toggle parameters display in chat buffer
CLOSED: [2025-04-26 Sat 08:12]
