...
Want to quickly access local running AI through Emacs and Ollama? This package provides a friendly Emacs interface for interacting with Ollama models and offers a convenient way to integrate Ollama's local LLM capabilities directly into your Emacs workflow with little or no configuration required!
...
A simple text information screen will be presented on the first opening of the chat (by selecting "o" from any of the menu), its just a bit of fun, but I wanted a quick start tutorial/assistant type of feel, here is what will be presented:
...
| TODO                                                | DONE                                    |
|-----------------------------------------------------+-----------------------------------------|
| Note sure there is conversation history with Claude | External AI to copy to register         |
| Add Texinfo to MELPA recipe                         | First request with no chat buffer fails |
| ChatGPT request/response too long                   |                                         |
| Check tags caching                                  |                                         |
...
| GGUF to use new syncing option                      |                                         |
| Save system prompts as part of session              |                                         |
| keybindings in ollama chat buffer are reserved      |                                         |

** TODO Note sure there is conversation history with Claude

** TODO Add Texinfo to MELPA recipe
...
** TODO System prompt not cleared on new session

** TODO Kill online AI process with cancel request

** TODO Remove fabric from Commands transient menu

** TODO GGUF to use new syncing option
...
| TODO                                 | DOING                             | DONE                                       |
|--------------------------------------+-----------------------------------+--------------------------------------------|
| Method for securely storing API keys | Core Params - suffix              | Core API - Delete Model                    |
| Merge history buffer functionality   | Show incoming ollama messages     | Core API - Copy a model                    |
| Core API - Embeddings                | Add fabric pattern/prompt support | Core API - stream                          |
| Core Params - format                 |                                   | Core API - Pull/Info/Pull/Delete a model   |
| Core Params - template               |                                   | Core API - Show running models             |
| Core Params - raw                    |                                   | Core API - Create a model from a GGUF file |
...
** TODO Method for securely storing API keys

** TODO Merge history buffer functionality
