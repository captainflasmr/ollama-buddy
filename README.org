...
| TODO                                                                         | DOING                                      | DONE                                                          |
|------------------------------------------------------------------------------+--------------------------------------------+---------------------------------------------------------------|
| transient-setup: Key sequence 1 0 starts with non-prefix key 1               | Kill online AI process with cancel request | As shell generally C-n to end of list to clear prompt history |
| Claude request and connection can sometimes take time and it blocks!         | GGUF to use new syncing option             | Add Texinfo to MELPA recipe                                   |
| =C-a/M-m= beginning-of-line to jump to prompt start like comint              |                                            | System prompt not cleared on new session                      |
| Pushing hundreds or even thousands of lines to the chat buffer takes a while |                                            | Remove fabric from Commands transient menu                    |
...
** TODO =C-a/M-m= beginning-of-line to jump to prompt start like comint
...
| TODO                               | DOING                              | DONE                                       |
|------------------------------------+------------------------------------+--------------------------------------------|
| Added gemini?                      | Core Params - suffix               | Method for securely storing API keys       |
| Merge history buffer functionality | Show incoming ollama messages      | Core API - Delete Model                    |
| Core API - Embeddings              | Add chatGPT awesome prompt support | Core API - Copy a model                    |
| Core Params - format               | Add fabric pattern/prompt support  | Core API - stream                          |
| Core Params - template             | ChatGPT support                    | Core API - Pull/Info/Pull/Delete a model   |
| Core Params - raw                  | Claude support                     | Core API - Show running models             |
