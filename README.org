...
Just put in some queries and show that we can step back through previously input, swap models and resubmit
...
If these files are put in the =ollama-buddy-roles-directory= then the role selection menu will pass through completing-read, and prsent the following:
