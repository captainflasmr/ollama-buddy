...
- Can now change models from chat buffer using C-c C-k
...
Good ollama connection status visibility is maintained by the following:

1. Introduced status caching:
...
If monitoring is desired, so for example it will consistently pick up ollama going online and office set the following:
