...
- PROMPT>> how many fingers do I have?
  ...
  Also they are now available in the equivalent letter named registers
...
With the new *multishot execution*, you can now send a prompt to multiple models in sequence, compare their responses, and easily navigate between them.  
...
1. Press *=C-c C-l=* to start a multishot session.  
...
4. The responses will be saved to registers of the same letter names for recalling later.
