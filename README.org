...
| TODO                                                                         | DOING                                      | DONE                                                                  |
|------------------------------------------------------------------------------+--------------------------------------------+-----------------------------------------------------------------------|
| Modifying a parameter internal defaults would flag as not modified           | chat ollama names to include prefix        | Still some encoding issues on returns from online AI                  |
| transient-setup: Key sequence 1 0 starts with non-prefix key 1               | Kill online AI process with cancel request | Conversation history to display with Claude/ChatGPT                   |
| No streaming with hiding reasoning corrupts output                           | GGUF to use new syncing option             | I think some hard coded Claude models currently don't work            |
...
| TODO                                                          | DOING                                       | DONE                                         |
|---------------------------------------------------------------+---------------------------------------------+----------------------------------------------|
| Unload all models?, to free up RAM if you are switching alot? | Add option to hide reasoning in chat buffer | Show token parameters as part of token stats |
| Autoload intelligently? to speed up initial reponse times?    | Core Params - suffix                        | Autogenerate session save name               |
| Core API - Embeddings                                         |                                             | Add gemini?                                  |
