...
- Add authentication support via =ollama signin= / =ollama signout= flow
- Detect and handle cloud model suffix (=-cloud=) in model names
- Display cloud vs local indicator in status line
