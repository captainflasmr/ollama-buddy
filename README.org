...
| TODO                                                                         | DOING                                      | DONE                                                                  |
|------------------------------------------------------------------------------+--------------------------------------------+-----------------------------------------------------------------------|
| chat ollama names to include prefix                                          | Kill online AI process with cancel request | Conversation history to display with Claude/ChatGPT                   |
| transient-setup: Key sequence 1 0 starts with non-prefix key 1               | GGUF to use new syncing option             | I think some hard coded Claude models currently don't work            |
...
| Still some encoding issues on returns from online AI                         |                                            | As shell generally C-n to end of list to clear prompt history         |
| ChatGPT request/response too long                                            |                                            | Add Texinfo to MELPA recipe                                           |
| Check tags caching                                                           |                                            | System prompt not cleared on new session                              |
| Save system prompts as part of session                                       |                                            | Remove fabric from Commands transient menu                            |
| keybindings in ollama chat buffer are reserved                               |                                            | External AI to copy to register                                       |
|                                                                              |                                            | First request with no chat buffer fails                               |

** TODO chat ollama names to include prefix
...
| TODO                                                          | DOING                | DONE                                       |
|---------------------------------------------------------------+----------------------+--------------------------------------------|
| Show processing / response time                               | Core Params - suffix | Autogenerate session save name             |
| Unload all models?, to free up RAM if you are switching alot? |                      | Add gemini?                                |
