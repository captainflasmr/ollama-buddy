...
- *Switch directory from chat* -- =/cd= slash command switches the working directory without leaving the chat; auto-loads project summary if available.
- *Non-blocking model info* -- Model capability fetching is now fully asynchronous; no more Emacs freezes with cloud models.
- *Error cleanup* -- "Working..." status is properly cleared on errors like "prompt too long", with prompt area restored.
- *Tool call arguments in headings* -- Folded tool call headings now show the JSON arguments inline for quick reference.
- *Removed Fabric and Awesome* -- Removed =ollama-buddy-fabric.el= and =ollama-buddy-awesome.el= modules to reduce maintenance burden.
