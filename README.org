...
- *Benchmark All Models* (=C-c u=) -- Send a configurable prompt to every available model via multishot, building token usage statistics for all accessible models in one command.  Embedding models are filtered out.
- *Multishot Per-Model Timeout* -- Each model in a multishot/benchmark sequence gets a configurable timeout (default 120s).  If exceeded, the request is cancelled and the sequence continues with the next model.
