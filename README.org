...
- *Pandoc Markdown to Org Conversion* -- Optionally use Pandoc for higher quality markdown to org-mode conversion (=ollama-buddy-use-pandoc-for-md-to-org=)
...
- *Smart Project Integration* -- Deep context awareness using Emacs' built-in `project.el`.  Easily attach any file from the current project or automatically pull in standard files like README, TODO, and CHANGELOG.
- *Project Slash Command* (=/project=) -- A new chat command to quickly search and attach files from the current project using completion.
- *Project Transient Actions* -- The Attachments menu (=C-c O a=) now includes project-specific actions when working inside a project.
