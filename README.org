...
| TODO                                                                         | DOING                                                  | DONE                                                                  |
|------------------------------------------------------------------------------+--------------------------------------------------------+-----------------------------------------------------------------------|
| Vision image with spaces not processed as image                              | Kill online AI process with cancel request             | Save system prompts as part of session                                |
| Modifying a parameter internal defaults would flag as not modified           | GGUF to use new syncing option                         | chat ollama names to include prefix                                   |
| transient-setup: Key sequence 1 0 starts with non-prefix key 1               | sometimes chat output with filter is getting clobbered | Still some encoding issues on returns from online AI                  |
...
| keybindings in ollama chat buffer are reserved                               |                                                        | Add Texinfo to MELPA recipe                                           |
...
** TODO Vision image with spaces not processed as image
