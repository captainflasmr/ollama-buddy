...
- *RAG* -- Index local documents and source code, retrieve relevant context for conversations via semantic search
- *Lightweight* -- No external dependencies; a minified 200-line version (=ollama-buddy-mini=) is also available

* What's New (2.5.0)

- *RAG Support* -- New =ollama-buddy-rag.el= module: index directories, search with embeddings, attach relevant context to conversations
- *Async Embedding* -- RAG indexing runs asynchronously with progress in the status line
- *Embedding Models* -- =nomic-embed-text=, =mxbai-embed-large=, =all-minilm= in recommended models
- *RAG Transient Menu* -- =C-c O= → =r= for index, search, attach, and management
- *49 RAG Tests* -- Full test coverage: chunking, vector math, index management, search roundtrip
