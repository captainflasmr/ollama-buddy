...
An Emacs package for interacting with local LLMs via [[https://ollama.ai/][Ollama]], with support for remote providers (OpenAI, Claude, Gemini, Grok, Copilot, Codestral, DeepSeek, OpenRouter, OpenCode Go) and Ollama Cloud models. Requires Emacs 29.1+.
...
- *Standalone Launch* -- =M-x ollama-buddy-launch-external= (or transient =v=) launches an agent from the current directory without requiring the chat buffer.  In Dired, opens in the displayed directory.
