...
- *Remote provider refactor* -- All 6 remote provider modules now share common infrastructure via =ollama-buddy-remote.el=, eliminating ~1400 lines of duplicated code. No user-facing changes; all providers work exactly as before. Adding a new OpenAI-compatible provider now requires ~100 lines instead of ~350.
