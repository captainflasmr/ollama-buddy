...
- *=@= Autocomplete* -- Typing =@= in the prompt area triggers =completing-read= with available inline commands (=search=, =rag=, =file=). Inserts syntax template with cursor inside parens. =C-g= cancels.
- *=ollama-buddy-at-commands=* -- Customizable alist for defining =@= command templates, extensible for future inline commands
- *Inline =@file(path)=* -- Attach files to context inline: =@file(/path/to/file)=. Paths expanded on send; works with =dired-copy-filename-as-kill=.
- *=/= Slash Commands* -- Typing =/= in the prompt area triggers =completing-read= with action commands (=model=, =copy=, =retry=, =tone=, =fabric=, =awesome=, =streaming=, =reset=, and more). Execute immediately; =C-g= inserts literal =/=.
- *=ollama-buddy-slash-commands=* -- Customizable alist for defining =/= slash command actions
...
(use-package ollama-buddy
  ...
  (setq ollama-buddy-default-model "kimi-k2.5:cloud")
  ;; key setup
  ...
  ;; not currently including remote LLMs
  ;; (require 'ollama-buddy-openai nil t)
  ...
  (setq ollama-buddy-tools-safe-mode nil)
  ...
  (ollama-buddy-update-menu-entry 'describe-code     :model "kimi-k2.5:cloud")
  (ollama-buddy-update-menu-entry 'dictionary-lookup :model "kimi-k2.5:cloud")
  (ollama-buddy-update-menu-entry 'synonym           :model "kimi-k2.5:cloud")
  ...
  ;; dired integration for attaching files
...
*** Refactor external LLMs to eliminate duplicate code

*** Pull the Ollama version for more API compatibility
