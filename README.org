...
Added expermental openai support!

Yes thats right, I said I never would do it, and of course this package is very much going to be still =ollama= centric, but I thought I would just sneak in some rudimentary ChatGPT support.

It is very simple, I haven't managed to get streaming working, so Emacs will block and wait for a single response and it isn't fully integrated into the ollama buddy full chat experience yet, oh, and it is also using =curl= as it simplifies certain aspects for me.

By default, openai/ChatGPT will not be enabled, so anyone wanting to just use a local LLM through =ollama= can continue as before, but you can sneak in some experiental ChatGPT support now by adding the following to your Emacs config:
...
and then the chat will present a list of ChatGPT models to choose from, with a single prompt running synchronously, so you will have to wait for a response and Emacs will temporarily pause.  Like I said, this is just experimental to prove a concept. I currently use ChatGPT quite a lot from within Emacs and would like to use the =ollama-buddy= interface if possible.
...
** TODO Transient initially not defined

#+begin_src 
transient-setup: Suffix command ollama-buddy--open-chat is not defined or autoloaded
#+end_src

** TODO After a multishot I sometimes get:

#+begin_src 
error in process filter: Args out of range: "aef", 3
error in process filter: let*: Args out of range: "aef", 3
error in process filter: Args out of range: "aef", 3
#+end_src
