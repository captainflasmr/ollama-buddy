...
- *Thinking Peek* -- Thinking tokens stream into a folded =*** Thinking= heading; press =TAB= to peek at accumulated content mid-stream, =TAB= again to re-fold.
- *Rewind* -- Navigate to a prompt heading and press =C-u C-u C-c C-c= to rewind the conversation to that point; falls back to =completing-read= when not on a heading; the prompt text is pre-filled for editing and resending.
