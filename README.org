...
- *RAG Transient Menu* -- =C-c r= opens RAG transient menu; also via =C-c O= → =r=
- *Inline @rag()* -- Use =@rag(query)= in prompts to automatically search and attach RAG context, like =@search()= for web search
- *System Prompt Reset* -- =C-c C-r= to reset system prompt; also via =C-c O= → =C-r=
- *55 RAG Tests* -- Full test coverage: chunking, vector math, index management, search roundtrip, inline processing
