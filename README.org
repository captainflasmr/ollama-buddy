...
Polish and new inspection capability for in-buffer replace mode.

- *Inline diff view* (=C-c d=) -- After a rewrite completes, press =C-c d= to insert the original text immediately below the new text, with word-level smerge highlighting: green on added/changed words in the new region, red strikethrough on removed words in the original block. Press =C-c d= again to hide it; accept or reject removes it automatically.
- *Automatic clean-output tone* -- When in-buffer replace is active, a "Return only the content" instruction is automatically prepended to the effective system prompt, keeping responses free of introductory phrases and commentary.
- *Code fence stripping* -- Markdown ``` fences (with optional language tag) are stripped from the response before it is placed in the buffer.
- *=C-c W=* -- New chat-buffer binding to toggle in-buffer replace without opening the transient menu.
- *Tighter command prompts* -- Region commands (Refactor, Git Commit, Describe Code, Dictionary Lookup, Synonym) now say "Return ONLY …" to avoid preamble when results are inserted directly into source code.

* What's New (2.8.0)

Initial in-buffer replacement mode — streams LLM rewrites directly into the source buffer rather than the chat buffer. The feature is off by default; the existing chat buffer workflow is completely unchanged.

- *In-buffer replace mode* -- Toggle =W= in the transient Actions menu (or =M-x ollama-buddy-toggle-in-buffer-replace=) to activate. With a region selected, any role command (Refactor, Proofread, etc.) will:
  1. Replace the selection with a =​[Rewriting...]=​ placeholder
  2. Stream the model's response back into the source buffer with a green highlight
  3. Activate =C-c C-c= (accept) / =C-c C-k= (reject/restore original) when the stream finishes
  - Cancelling mid-stream with =C-c C-k= stops the network process and restores the original text
  - The =✎= symbol appears in the header and prompt line while the mode is active
  - Mode line shows =​ [Rewrite?]=​ while a decision is pending

* What's New (2.7.3)

- *Collapsible thinking blocks* -- Reasoning model output (from =<think>= tags or the DeepSeek API =message.thinking= field) now renders as a native org-mode heading (=*** ✦ Think=) that folds automatically when the model finishes reasoning. A =*** ✦ Response= heading marks the start of the actual answer. Use =C-c V= to toggle all thinking blocks in the buffer, or =TAB= on any =*** ✦ Think= heading to toggle it individually. The =✦= symbol appears in the header line and prompt line when the active model supports thinking/reasoning.
