...
The main menu is the entry point for interacting with Ollama Buddy. It groups commands into logical sections for better accessibility:

- *Chat Commands*: Open a chat (=o=), send a prompt (=RET=), access help (=h=), and cancel requests (=k=).
- *Prompt Management*: Send a region (=l=), set a system prompt (=s=), view the current prompt (=C-s=), and reset it (=r=).
- *Model Controls*: Switch models (=m=), view status (=v=), inspect model information (=i=), and use multishot prompts (=M=).
- *Roles & Patterns*: Manage conversation roles (=R=), open the roles directory (=D=), and access Fabric patterns (=f=).

Additional sections allow for toggling display options, managing session history, handling saved sessions, and editing parameters.

** Fabric Patterns Menu

Fabric patterns integrate structured interaction templates into the chat. The Fabric menu provides actions such as:

- *Pattern Actions*: Send input using a pattern (=s=), list available patterns (=l=), and view details (=v=).
- *Synchronization*: Sync the latest patterns (=S=), populate cached patterns (=P=), and perform initial setup (=I=).
- *Pattern Categories*: Quickly switch between predefined patterns for coding (=c=), writing (=w=), analysis (=a=), and general use (=u=).

** Parameter Menu

The parameter menu allows customization of generation settings, model behavior, and resource allocation:

- *Generation Options*: Adjust parameters like temperature, top-k, top-p sampling, and frequency penalties.
- *Mirostat Settings*: Modify adaptive sampling controls such as tau and eta.
- *Resource Allocation*: Set context window size, GPU usage, and batch processing limits.
- *Memory and Execution*: Control MMAP/MLock usage, thread allocation, and NUMA optimization.

Users can also apply pre-configured parameter profiles such as *Default*, *Creative*, or *Precise*.

** Commands Menu

For specialized tasks, the commands menu provides quick access to built-in actions:

- *Code Operations*: Refactor code (=r=), describe a code snippet (=d=), and generate Git commit messages (=g=).
- *Language Processing*: Perform dictionary lookups (=l=), synonym searches (=s=), and proofread text (=p=).
- *Pattern-Based Interactions*: Access Fabric-based workflows for different use cases (=f=, =u=, =c=).
