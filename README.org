...
Inline ghost-text code completions — a first, simple implementation of Copilot-style completions.

- *=ollama-buddy-completion=* (new module) -- Manual inline completions that appear as dimmed ghost text at point.  This is an initial, straightforward implementation; quality depends heavily on the model — dedicated code models (=codellama=, =qwen2.5-coder=) work best, while general-purpose or thinking models may produce verbose suggestions.
  - =C-c TAB= to trigger, =TAB= to accept, any other key dismisses
  - Fill-in-the-middle (FIM): sends prefix *and* suffix context
  - Thinking model fallback, markdown fence stripping, multi-line display
  - Toggle with =M-x ollama-buddy-completion-mode=
