...
- *In-Buffer Replace (initial)* -- An early-stage mode that streams LLM output directly back into the source buffer, replacing the selected region in-place with accept/reject confirmation
- *Lightweight* -- No external dependencies; a minified 200-line version (=ollama-buddy-mini=) is also available

* What's New (2.8.0)

This release adds an initial pass at in-buffer replacement — a first attempt at bringing LLM rewrites directly into the source buffer rather than the chat buffer. The feature is intentionally conservative: it does not change any existing workflow, prompt handling, or command definitions. When toggled off (the default) everything behaves exactly as before.
