...
- =C-c A= - Access the full attachment menu via a transient interface
...
The welcome screen is controlled by =ollama-buddy-full-welcome-enabled= (default: t).  To show only the essential keybindings:

#+begin_src elisp
(setq ollama-buddy-full-welcome-enabled nil)
#+end_src

Here is an example of the minimal welcome screen (=full-welcome-enabled= nil):
...
and the full welcome screen (default, =full-welcome-enabled= t):
...
- *Save session* with =ollama-buddy-sessions-save= (or =C-c S=) Preserve your current conversation with a custom name
